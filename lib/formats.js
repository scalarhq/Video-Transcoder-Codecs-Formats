...
const init = CODEC_TYPES => {
  const errs = [];
  fs.readdirSync(path.join(__dirname, DIR_NAME)).filter(file => file.slice(file.length - 3) === ".js" && file !== path.basename(__filename)).map(file => [file.slice(0, -3).replace("-", "").replace(".", "").replace(" ", "_").toUpperCase(), require(path.join(__dirname, DIR_NAME, file))(CODEC_TYPES)]).forEach(e => {
